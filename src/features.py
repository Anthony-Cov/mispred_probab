...
import gc
import src
from src.utils import Norm01, Nback, MLS 
...
def CEmbDim(dat): #То же по-быстрому с C++ процедурой EmbDim.so'
    ...
    if len(dat)>1000:
        ...
        y=dat
    emd = ctypes.CDLL(split(getfile(src.utils))[0]+'/EmbDim.so')
...
def CCorrent(dat): #Корреляционная энтропия по-быстрому с C++ процедурой CorrEntr.cpp
    ...
    if len(dat)>1000:
        ...
        y=dat
    emd = ctypes.CDLL(split(getfile(src.utils))[0]+'/CorrEntr.so')
...
def CHurst(dat): #То же по-быстрому с C++ процедурой HurstExp.so
    ...
    if len(dat)>1000:
        ...
        y=dat
    he = ctypes.CDLL(split(getfile(src.utils))[0]+'/HurstExp.so')
